...
#else
  uint64_t test_duration_seconds;
...
#else
  test_duration_seconds = (uint64_t) test_duration_cycles / (uint64_t) CLOCKS_PER_SEC;
  iterations_per_second = (uint64_t) RUNS / test_duration_seconds;
#endif
...
#else
  ee_printf("Seconds elapsed: %llu\n", test_duration_seconds);
  ee_printf("Iterations per second: %llu\n", iterations_per_second);
#endif
